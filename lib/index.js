...
// —— 自定义状态形象 / 新增音效（2026-09 增补）——
// 优先包内 assets；其次用户皮肤目录(即使插件包被更新重建仍可用)；最后回退默认鲸鱼。
const STATE_IMAGE_SETS = {
  angry: [path.join(PACKAGE_ROOT, 'assets', 'angry.png'), path.join(PACKAGE_ROOT, 'assets', 'DSniang1.png')],
  full: [path.join(PACKAGE_ROOT, 'assets', 'full.png'), path.join(PACKAGE_ROOT, 'assets', 'DSniang1.png')],
}
const EXTRA_SOUND_SETS = {
  normal1: [path.join(PACKAGE_ROOT, 'assets', 'normal1.wav')],
  normal2: [path.join(PACKAGE_ROOT, 'assets', 'normal2.wav')],
  angry1: [path.join(PACKAGE_ROOT, 'assets', 'angry1.wav')],
  angry2: [path.join(PACKAGE_ROOT, 'assets', 'angry2.wav')],
  full1: [path.join(PACKAGE_ROOT, 'assets', 'full1.wav')],
  full2: [path.join(PACKAGE_ROOT, 'assets', 'full2.wav')],
}
// 「吃」状态的透明动画(WebP, 带alpha)：优先包内，其次用户目录
const EAT_WEBP_CANDIDATES = [
  path.join(PACKAGE_ROOT, 'assets', 'eat.webp'),
]
// 三口表情包：包内 assets/expressions 存放压缩后的表情(webp)，随机出图
const EXPR_DIR = path.join(PACKAGE_ROOT, 'assets', 'expressions')
const EXPR_NAME_OK = /^[\w\u4e00-\u9fff-]+\.(webp|png|gif|jpg|jpeg)$/i
...
var css = [
  ...
  '.dshwv-msg{position:absolute;left:44%;top:31%;transform:translate(-50%,-50%);z-index:3;background:rgba(255,255,255,.97);border:calc(var(--dshw-u) * 3) solid #203170;border-radius:999px;padding:calc(var(--dshw-u) * 12) calc(var(--dshw-u) * 32);color:#203170;font-size:calc(var(--dshw-u) * 56);font-weight:700;white-space:nowrap;opacity:0;transition:opacity .16s ease;pointer-events:none;line-height:1.1;box-shadow:0 4px 12px rgba(0,0,0,.15)}',
  '.dshwv-msg.dshwv-msg-on{opacity:1}',
  '.dshwv-root.dshwv-left .dshwv-msg{transform:translate(-50%,-50%) scaleX(-1)}',
  '.dshwv-img{transition:opacity .25s ease}'
].join('\\n')
...
soundSelect.appendChild(soundOpt('voice', '语音·点击(嗯？)'))
...
// —— 余额提醒(吃撑状态)：累计消耗满 X 元后角色变“太撑了”并提示 ——
...
remindToggle.title = '自上次提醒起累计消耗达到设定金额时，角色进入“吃撑”状态提醒'
remindToggle.addEventListener('change', function () { setRemindOn(remindToggle.checked) })
var remindYuanInput = document.createElement('input')
remindYuanInput.type = 'number'
remindYuanInput.min = '0.1'
remindYuanInput.max = '1000'
remindYuanInput.step = '0.1'
remindYuanInput.className = 'dshwv-number'
remindYuanInput.value = '10'
remindYuanInput.title = '0.1~1000，可精确到 0.1；点击上下箭头按 0.1 步进'
remindYuanInput.addEventListener('input', function () {
  var n = Number(remindYuanInput.value)
  if (remindPreset && isFinite(n)) {
    var hit = ''
    var presets = ['0.1', '1', '10', '100', '1000']
    for (var i = 0; i < presets.length; i++) if (Math.abs(Number(presets[i]) - n) < 1e-9) { hit = presets[i]; break }
    remindPreset.value = hit || 'custom'
  }
})
remindYuanInput.addEventListener('change', function () { setRemindYuan(remindYuanInput.value) })
remindYuanInput.addEventListener('blur', function () { setRemindYuan(remindYuanInput.value) })
var remindPreset = document.createElement('select')
remindPreset.className = 'dshwv-sound'
remindPreset.appendChild(soundOpt('0.1', '0.1'))
remindPreset.appendChild(soundOpt('1', '1'))
remindPreset.appendChild(soundOpt('10', '10'))
remindPreset.appendChild(soundOpt('100', '100'))
remindPreset.appendChild(soundOpt('1000', '1000'))
remindPreset.appendChild(soundOpt('custom', '自定义…'))
remindPreset.value = '10'
remindPreset.title = '固定档位，或选“自定义…”手动输入 0.1~1000'
remindPreset.addEventListener('change', function () {
  if (remindPreset.value === 'custom') { remindYuanInput.focus() }
  else setRemindYuan(remindPreset.value)
})
var rowRemind = menuRow()
rowRemind.appendChild(menuLabel('余额提醒'))
rowRemind.appendChild(remindToggle)
rowRemind.appendChild(remindPreset)
rowRemind.appendChild(remindYuanInput)
rowRemind.appendChild(menuLabel('元'))
...
var RANDOM_GROUPS = [
  ...
  { w: 7, lines: function () { return singleCenter('B', pickOne(['我超棒的！', '别烦我！'])) } },
  { w: 7, lines: function () { return singleCenter('A', pickOne(['别点这么快…我要吃撑啦！', '我…我就想多吃几口，不行吗？', '好饿…今天才喂了我几口余额呀？', '吃得好饱…先打个盹~', '哼！你再连点试试？', '三口=三张嘴，当然吃得多啦！']), '', true) } },
  { w: 10, lines: function () { return { gif: true } } },
  { w: 3, lines: function () { return singleCenter('A', pickOne(['又让我吃了这么多…这月饭钱要超支啦！', '余额少一分，肚皮鼓一分~', '你负责喂，我负责吃，账房先记上~']), '', true) } },
  { w: 1, lines: function () { return singleCenter('B', '再来一口嘛~') } },
]
...
function applyBubbleLines(lines) {
  if (lines && lines.gif) {
    ...
    if (gifFailed) {
      ...
      showExprOrGif() // 随机三口表情/动图（无可用时回退 rua.gif）
...
function remindFmt(v) {
  var n = Math.round(Number(v) * 10) / 10
  return String(n)
}
function normalizeRemindYuan(v) {
  var n = Number(v)
  if (!isFinite(n) || n <= 0) n = remindYuan
  n = Math.max(0.1, Math.min(1000, n)) // 0.1 ~ 1000
  return Math.round(n * 10) / 10       // 精确到 0.1
}
function syncRemindPreset() {
  if (!remindPreset) return
  var presets = ['0.1', '1', '10', '100', '1000']
  var hit = ''
  for (var i = 0; i < presets.length; i++) {
    if (Math.abs(Number(presets[i]) - remindYuan) < 1e-9) { hit = presets[i]; break }
  }
  remindPreset.value = hit || 'custom'
}
function setRemindOn(v) {
  ...
  remindYuanInput.disabled = !remindOn
  if (remindPreset) remindPreset.disabled = !remindOn
  saveConfig()
}
function setRemindYuan(v) {
  remindYuan = normalizeRemindYuan(v)
  remindYuanInput.value = remindFmt(remindYuan)
  syncRemindPreset()
  saveConfig()
}
function setScale(v) {
...
function setSoundSet(v) {
  if (v === 'duck' || v === 'fx1' || v === 'voice') soundSet = v
  else soundSet = 'voice'
  soundSelect.value = soundSet
  applySoundSet()
  saveConfig()
  if (soundSet === 'voice') scheduleIdle()
}
...
function applySoundSet() {
  try {
    if (soundSet === 'voice') {
      // 语音模式下不使用按压 mp3（点击语音走 extra.wav）
      pressAudio = null
      releaseAudio = null
      return
    }
...
function applyMenu(open) {
  // 以真实 DOM 状态为唯一事实来源，每次强制同步，避免 menuOpen 与显示状态
  // 失步后(如点击被动画/其它事件打断)出现“怎么点都不弹菜单”的情况。
  menuOpen = open
  if (open) {
    positionMenu()
...
function toggleMenu() {
  applyMenu(!menuBox.classList.contains('dshwv-menu-open'))
}
function closeMenu() {
  applyMenu(false)
  root.style.transition = ''
...
function onDocPointerUp(e) {
  ...
  // 但菜单按钮/菜单内部的交互控件必须放行（否则点菜单会失灵）
  try {
    var t = e.target && e.target.closest ? e.target.closest('.dshwv-bubble,.dshwv-menu,.dshwv-menu-btn') : null
    if (!t && isWhaleHit(e)) { e.preventDefault(); e.stopPropagation() }
  } catch (err) {}
  endDrag(e, true)
}
...
function onDocClickStopper(e) {
  ...
  // 菜单按钮/菜单内部是交互 UI：一律放行，避免点击被鲸鱼像素吞掉导致
  // 菜单“怎么点都不弹/点了没反应”。
  try {
    if (e.target && e.target.closest && e.target.closest('.dshwv-bubble,.dshwv-menu,.dshwv-menu-btn')) return
  } catch (err) {}
  if (!isWhaleHit(e)) return
  try { e.preventDefault(); e.stopPropagation() } catch (err) {}
}
...
function endDrag(e, clickAllowed) {
  ...
  if (clickAllowed && !drag.moved) { registerWhaleClick(); showBubble(); refresh(true); return }
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.soundSet === 'string') {
      soundSet = d.soundSet === 'duck' || d.soundSet === 'fx1' || d.soundSet === 'voice' ? d.soundSet : 'voice'
      soundSelect.value = soundSet
      applySoundSet()
      if (soundSet === 'voice') scheduleIdle()
    }
    ...
    if (d && typeof d.remindYuan === 'number') {
      remindYuan = normalizeRemindYuan(d.remindYuan)
      remindYuanInput.value = remindFmt(remindYuan)
      syncRemindPreset()
    }
      ...
      .then(function (d) {
        ...
        if (d.seq > lastCostSeq) {
          ...
          if (d.turn !== null && d.amount !== null) {
            ...
            showCostBubble(amt)
            // 每轮消耗提示出现时同步播“吃.wav”（替代闲置随机音效；与弹泡条件一致）
            if (bubbleOn && turnCostOn && soundOn && soundVol > 0) {
              playVoice('normal2')
              tryEatAnim()
            }
            // 累计提醒：自上次提醒起累计消耗（每轮真实金额累加）
            if (remindOn && isFinite(amt) && amt > 0) consumedAcc += amt
          }
        }
        // 无状态时，达到提醒线 → 吃撑状态（弹“已吞X元”；音效延迟 1.6s 避免与“吃.wav”叠音）
        if (remindOn && consumedAcc >= remindYuan && !kindActive) {
          consumedAcc = consumedAcc - remindYuan
          enterState('full', '已吞' + fmtYuan(remindYuan) + '元', ['full1', 'full2'], 1600)
        }
      })
      .catch(function () {})
  } catch (err) {}
...
// —— 语音 / 状态 / 提醒实现（2026-09 增补）——
function fmtYuan(v) { return String(Math.round(Number(v) * 100) / 100) }
function showMsg(text, ms) {
  if (!msgEl) return
  if (msgTimer) { clearTimeout(msgTimer); msgTimer = null }
  msgEl.textContent = text
  msgEl.classList.add('dshwv-msg-on')
  msgTimer = setTimeout(function () {
    msgTimer = null
    msgEl.classList.remove('dshwv-msg-on')
  }, ms || 2800)
}
var curVoice = null
var curVoiceName = ''
function playVoice(name) {
  ...
  stopVoice() // 先停掉上一条语音，避免叠音
  curVoiceName = name
  try {
    var au = new Audio('/dsh-whale/extra.wav?n=' + encodeURIComponent(name))
    au.volume = soundVol
    curVoice = au
    var pp = au.play()
...
function stopVoice() {
  if (curVoice) {
    try { curVoice.pause(); curVoice.currentTime = 0 } catch (err) {}
    curVoice = null
  }
  curVoiceName = ''
}
function pickVoice(pair) {
  var idx = Math.floor(Math.random() * pair.length)
  if (pair.length > 1 && pair[idx] === lastVoiceName) idx = (idx + 1) % pair.length
  return pair[idx]
}
function setCharImage(src) {
  img.src = src
  // 命中热区与当前显示的图保持一致（状态图同为 610×610 构图）
  try {
    var probe = new Image()
    probe.onload = function () {
      try {
...
function scheduleIdle() {
  // 闲置不再自动发声（吃.wav 改在“每轮消耗提示”时播放）
}
function backNormal() {
  if (!kindActive) return
  kindActive = ''
  if (kindTimer) { clearTimeout(kindTimer); kindTimer = null }
  setCharImage(IMG_URL)
}
function showFullReminder(text) {
  // 与原版“每轮消耗提示”完全同款的气泡样式，仅更换文案
  if (!bubbleOn) return
  if (costBubbleTimer) { clearTimeout(costBubbleTimer); costBubbleTimer = null }
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
  if (gifFadeTimer) { clearTimeout(gifFadeTimer); gifFadeTimer = null }
  ...
  labelEl.style.display = ''
  labelEl.className = 'dshwv-label'
  labelEl.textContent = '余额提醒:'
  labelEl.style.color = ''
  amountEl.style.display = ''
  amountEl.className = 'dshwv-amount'
  amountEl.textContent = text // 形如“已吞10元”
  amountEl.style.color = '#e0433f'
  hintEl.style.display = 'none'
  hintEl.textContent = ''
  hintEl.style.color = ''
  ...
  textBox.style.opacity = ''
  bubbleBox.classList.add('dshwv-bubble-open')
  costBubbleTimer = setTimeout(hideCostBubble, 3200)
}
var eatAnimOk = null // null=未知 true=可用 false=失败(只播音效)
function tryEatAnim() {
  // “吃”透明动画：约2.1s WebP；失败则静默回退为仅音效
  if (kindActive || eatAnimOk === false) return
  kindActive = 'eat'
  var onOk = function () { eatAnimOk = true }
  var onFail = function () {
    eatAnimOk = false
    img.onload = null
    img.onerror = null
  ...
  img.onload = onOk
  img.onerror = onFail
  img.src = '/dsh-whale/eat.webp'
  if (kindTimer) { clearTimeout(kindTimer); kindTimer = null }
  // 一遍时长≈63帧×33ms≈2079ms；在下一遍首帧出现前(2050ms)即恢复，避免重复开头画面
  kindTimer = setTimeout(stopEatAnim, 2050)
}
function stopEatAnim() {
  if (kindActive !== 'eat') return
  kindActive = ''
  if (kindTimer) { clearTimeout(kindTimer); kindTimer = null }
  img.onload = null
  img.onerror = null
  setCharImage(IMG_URL) // 恢复静态皮肤与命中热区
}
function enterState(kind, msg, voices, voiceDelay) {
  // full/angry 互斥；生气期间可再次触发；吃动画状态可被任何状态打断
  if (kindActive && kindActive !== kind && kindActive !== 'eat') return
  if (kindActive === 'eat') stopEatAnim()
  if (kindTimer) { clearTimeout(kindTimer); kindTimer = null }
  kindActive = kind
  setCharImage('/dsh-whale/state.png?st=' + kind)
  if (kind === 'full' && msg) showFullReminder(msg) // 生气不弹字幕；吃撑用原版气泡弹“已吞X元”
  stopVoice()
  function fireVoice() {
    if (kindActive === kind) playVoice(pickVoice(voices))
  }
  if (voiceDelay && voiceDelay > 0) setTimeout(fireVoice, voiceDelay)
  else fireVoice()
  kindTimer = setTimeout(backNormal, 5000)
}
function registerWhaleClick() {
  var now = Date.now()
  clickStamps.push(now)
  while (clickStamps.length && now - clickStamps[0] > 2000) clickStamps.shift()
  // 频繁点击(2秒内第6次) → 生气状态(可重复触发)
  if (clickStamps.length >= 6) {
    clickStamps = []
    if (kindActive !== 'full') enterState('angry', '', ['angry1', 'angry2'], 0)
    return
  }
  // 普通点击：仅“嗯？”，生气/吃撑期间不播
  if (soundSet === 'voice' && !kindActive && soundOn) {
    playVoice('normal1')
  }
}
// —— 三口表情包（随机气泡表情）——
var exprs = []
var lastExpr = ''
var exprFallbackDone = false
function pickExpr() {
  if (!exprs.length) return ''
  var n = exprs[Math.floor(Math.random() * exprs.length)]
  if (exprs.length > 1 && n === lastExpr) n = exprs[(exprs.indexOf(n) + 1) % exprs.length]
  lastExpr = n
  return n
}
function showExprOrGif() {
  var useExpr = exprs.length > 0 && !exprFallbackDone
  var src
  if (useExpr) {
    src = '/dsh-whale/expr.bin?f=' + encodeURIComponent(pickExpr())
    gifEl.onerror = function () {
      // 随机表情加载失败：本会话内回退到 rua.gif，避免空白气泡
      if (!exprFallbackDone) {
        exprFallbackDone = true
        gifFailed = false
  ...
  gifEl.onload = function () { gifFailed = false }
  try { gifEl.src = src } catch (err) {}
}
try {
  fetch('/dsh-whale/exprs.json', { cache: 'no-store' })
    .then(function (r) { return r.json() })
    .then(function (d) { if (d && Array.isArray(d.files) && d.files.length) exprs = d.files })
    .catch(function () {})
} catch (err) {}
applySoundSet()
})()`
...
const inject = ['webServer', 'credentials', 'connection']

function apply(ctx) {
    // —— 浏览器信任栅栏（新版 DSH 安全要求：自定义路由需先过 requestRejection）——
      ...
      for (const p of SIZE_FILE_CANDIDATES) {
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            const ss = parsed.soundSet
            return {
              scale: parsed.scale,
              sound: parsed.sound !== false,
              vol: typeof parsed.vol === 'number' ? parsed.vol : 0.9,
              soundSet: ss === 'fx1' || ss === 'duck' || ss === 'voice' ? ss : 'voice',
              ...
              remindYuan: typeof parsed.remindYuan === 'number' && isFinite(parsed.remindYuan) && parsed.remindYuan > 0 ? Math.max(0.1, Math.min(1000, Math.round(parsed.remindYuan * 10) / 10)) : 10,
            }
          }
        } catch (err) {}
      }
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, extras) {
      ...
      const ss = soundSet === 'fx1' || soundSet === 'duck' || soundSet === 'voice' ? soundSet : 'voice'
      const ex = extras && typeof extras === 'object' ? extras : {}
      const remindOn = ex.remindOn !== false
      const remindYuan = typeof ex.remindYuan === 'number' && isFinite(ex.remindYuan) && ex.remindYuan > 0 ? Math.max(0.1, Math.min(1000, Math.round(ex.remindYuan * 10) / 10)) : 10
      const base = {
        scale: scale,
        sound: sound !== false,
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, { remindOn: parsed.remindOn !== false, remindYuan: parsed.remindYuan })
    ...
    function loadFileBytes(candidates) {
      for (const p of candidates) {
        try {
          const bytes = fs.readFileSync(p)
          if (bytes && bytes.length > 0) return bytes
    ...
    // 状态形象：生气/吃撑 时前端把 img.src 切到本路由
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/state.png',
      handler: (req, res) => {
        const st = queryParam(req.url, 'st')
        const candidates = (st === 'angry' || st === 'full') && STATE_IMAGE_SETS[st] ? STATE_IMAGE_SETS[st] : IMAGE_CANDIDATES
        const bytes = loadFileBytes(candidates)
        if (!bytes) {
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('state image unavailable')
          return
        }
        res.writeHead(200, {
          'Content-Type': 'image/png',
    ...
    // 新增语音：普通点击(嗯？/吃)、生气(干嘛/嗯)、吃撑(这也太多了/打嗝)
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/extra.wav',
      handler: (req, res) => {
        const n = queryParam(req.url, 'n')
        const candidates = EXTRA_SOUND_SETS[n]
        const bytes = candidates ? loadFileBytes(candidates) : null
        if (!bytes) {
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('sound unavailable')
          return
    ...
    // 「吃」状态透明动画 WebP（由 吃.mov 转码，带 alpha）
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/eat.webp',
      handler: (req, res) => {
        const bytes = loadFileBytes(EAT_WEBP_CANDIDATES)
        if (!bytes) {
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('eat animation unavailable')
          return
        }
        res.writeHead(200, {
    ...
    // 三口表情包：文件名列表 + 按名取图（随机表情用）
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/exprs.json',
      handler: (req, res) => {
        let files = []
        try {
          files = fs.readdirSync(EXPR_DIR)
            .filter((f) => EXPR_NAME_OK.test(f))
            .sort()
        } catch (err) {}
        res.writeHead(200, JSON_HEADERS)
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/expr.bin',
      handler: (req, res) => {
        const name = queryParam(req.url, 'f')
        if (!EXPR_NAME_OK.test(name || '')) {
          res.writeHead(400, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('bad name')
          return
        }
        const bytes = loadFileBytes([path.join(EXPR_DIR, name)])
        if (!bytes) {
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('expression not found')
          return
        }
        const ext = String(name.split('.').pop()).toLowerCase()
        const mime = ext === 'webp' ? 'image/webp' : ext === 'png' ? 'image/png' : ext === 'gif' ? 'image/gif' : 'image/jpeg'
        res.writeHead(200, {
          'Content-Type': mime,
          'Cache-Control': 'no-store',
          'Content-Length': String(bytes.length),
